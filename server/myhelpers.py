...
def readFileAllLines(filename, encoding:str=None):
    if encoding is None:
        encoding = getEncoding(filename)
    with open(filename, 'r', encoding=encoding, errors='replace') as file:
        return file.readlines()

def readFileAllText(filename, encoding:str=None):
    if encoding is None:
        encoding = getEncoding(filename)
    with open(filename, 'r', encoding=encoding, errors='replace') as file:
        return file.read()
...
def saveFileAllText(filename, text, encoding="utf-8"):
    with open(filename, 'w', encoding=encoding) as file:
        file.write(text)
        file.flush()

writeFileAllText = saveFileAllText
...
class ConfigHelper:
    ...
    def getIntFromConfig(self,key,section='1',defaultValue = 0):
        return self.config.getint(section, key, fallback=defaultValue)

    def getFloatFromConfig(self,key,section='1',defaultValue = 0):
